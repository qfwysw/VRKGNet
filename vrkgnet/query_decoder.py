...
@MODELS.register_module()
class SegInstQueryDecoder(BaseModule):
    ...
    def __init__(self, 
        ...
        self.out_sem = nn.Sequential(
                nn.Linear(self.num_queries_1dataset, self.num_queries_1dataset), nn.ReLU(),
                nn.Linear(self.num_queries_1dataset, num_classes_1dataset + 1))
    ...
    def _forward_head(self, queries, mask_feats, scene_names, last_flag):
        """Prediction head forward.
        ...
        Returns:
            Tuple:
                ...
                List[Tensor]: Attention masks of len batch_size,
                    each of shape (n_queries_i, n_points_i).
        """
        cls_preds, pred_scores, pred_masks, attn_masks, sem_preds = [], [], [], [], []
        for i in range(len(queries)):
            # 
            norm_query = self.out_norm(queries[i])
            ...
            pred_masks.append(pred_mask)
            if last_flag:
                sem_preds.append(self.out_sem(pred_mask.permute(1, 0)).permute(1, 0))
        sem_preds = sem_preds if last_flag else None
        return  cls_preds, pred_scores, pred_masks, attn_masks, sem_preds
    ...
    def forward_simple(self, x, scene_names):
        ...
        for i in range(len(self.cross_attn_layers)):
            ...
            queries = self.self_attn_layers[i](queries)
            queries = self.ffn_layers[i](queries)
        cls_preds, pred_scores, pred_masks, _, sem_preds = self._forward_head(
            queries, mask_feats, scene_names, last_flag=True)
    ...
    def forward_iter_pred(self, x, scene_names):
        ...
        mask_feats = [self.x_mask(y) for y in x]
        queries = self._get_queries(len(x), scene_names)
        cls_pred, pred_score, pred_mask, attn_mask, sem_pred = self._forward_head(
            queries, mask_feats, scene_names, last_flag=True)
        cls_preds.append(cls_pred)
        sem_preds.append(sem_pred)
        pred_scores.append(pred_score)
...
class OneDataQueryDecoder(BaseModule):
    def __init__(self, 
        ...
        self.cross_attn_layers = nn.ModuleList([])
        self.cross_attn_layers1 = nn.ModuleList([])
        self.self_attn_layers = nn.ModuleList([])
        self.ffn_layers = nn.ModuleList([])
        ...
        for i in range(num_layers):
            ...
            self.self_attn_layers.append(
                SelfAttentionLayer(d_model, num_heads, dropout))
            self.cross_attn_layers1.append(
                CrossAttentionLayer(
                    d_model, num_heads, dropout, fix_attention))
            self.ffn_layers.append(
                FFN(d_model, hidden_dim, dropout, activation_fn))
    ...
    def forward_simple(self, x, scene_names):
        ...
        for i in range(len(self.cross_attn_layers)):
            ...
            queries = self.self_attn_layers[i](queries)
            queries = self.ffn_layers[i](queries)
            inst_feats = self.cross_attn_layers1[i](queries, inst_feats)
            # queries = self.ffn_layers[i](queries)
        cls_preds, pred_scores, pred_masks, _ = self._forward_head(
        ...
        return dict(
            cls_preds=cls_preds,
            masks=pred_masks,
            scores=pred_scores), mask_feats[0]
    ...
    def forward_iter_pred(self, x, scene_names):
        ...
        for i in range(len(self.cross_attn_layers)):
            ...
            queries = self.ffn_layers[i](queries)
            inst_feats = self.cross_attn_layers1[i](queries, inst_feats)
            # queries = self.ffn_layers1[i](queries)
            cls_pred, pred_score, pred_mask, attn_mask = self._forward_head(
                queries, mask_feats, scene_names)
            cls_preds.append(cls_pred)
            pred_scores.append(pred_score)
        ...
        return dict(
            cls_preds=cls_preds[-1],
            masks=pred_masks[-1],   # [400, 37527]
            scores=pred_scores[-1],
            aux_outputs=aux_outputs), mask_feats[0]
